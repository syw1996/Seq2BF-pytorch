...
def build_translator(opt, report_score=True, logger=None, out_file=None):
    ...
    bk_fields, bk_model, _ = \
        onmt.model_builder.load_test_model(opt, dummy_opt.__dict__, model_path=opt.backward_model)
    ...
    translator = Translator(model, bk_model, fields, bk_fields, global_scorer=scorer,
...
class Translator(object):
    ...
    def __init__(self,
        ...
        self.model = model
        self.bk_model = bk_model
        self.fields = fields
        self.bk_fields = bk_fields
        self.n_best = n_best
    ...
    def translate_batch(self, model, batch, data, keyword=None, front_seq=None, fast=False):
        """
        ...
        with torch.no_grad():
            if fast:
                ...
                return self._translate_batch(model, batch, data, keyword, front_seq)
    ...
    def _translate_batch(self, model, batch, data, keyword, front_seq):
        ...
        for i in range(self.max_length):
            ...
            # in the decoder
            # inp: [batch_size, beam_size]
            inp = inp.unsqueeze(2)

            # backward添加keyword
            if keyword is not None and i == 1:
                for j in range(inp.size()[1]):
                    inp[0][j] = keyword
            
            # front_seq = None
            # forward解码时将部分pad替换成已生成的front_seq
            if front_seq is not None:
                if i > 0 and i <= len(front_seq):
                    for j in range(inp.size()[1]):
                        inp[0][j] = front_seq[i-1] 
            # print(inp)           

            # Run one step.
            dec_out, dec_states, attn = model.decoder(
                inp, memory_bank, dec_states, True,
                memory_lengths=memory_lengths,
                step=i)
        ...
        if "tgt" in batch.__dict__:
            ret["gold_score"] = self._run_target(model, batch, data)
        ret["batch"] = batch
    ...
    def _run_target(self, model, batch, data):
        data_type = data.data_type
        if data_type == 'text':
            _, src_lengths = batch.src
        else:
        ...
        enc_states, memory_bank = model.encoder(src, src_lengths)
        dec_states = \
            model.decoder.init_decoder_state(src, memory_bank, enc_states)
        ...
        dec_out, _, _ = model.decoder(
            tgt_in, memory_bank, dec_states, True, memory_lengths=src_lengths)

        tgt_pad = self.fields["tgt"].vocab.stoi[inputters.PAD_WORD]
        for dec, tgt in zip(dec_out, batch.tgt[1:].data):
            # Log prob of each word.
            out = model.generator.forward(dec)
            tgt = tgt.unsqueeze(1)
